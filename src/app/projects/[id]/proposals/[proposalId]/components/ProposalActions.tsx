...
import type { ProposalStatusCode } from '@/types/proposal/base';
...
export function ProposalActions() {
  ...
  useEffect(() => {
    const initializeProposal = async () => {
      ...
      try {
        ...
        setError(error instanceof Error ? error.message : 'Failed to initialize proposal');
        toast.error('Failed to initialize proposal');
      }
    };

    initializeProposal();
  }, [authLoading, user, router, isNewProposal, proposalId, projectId, fetchStatuses, setProposal, setError, isInitialized, statuses.length]);
  ...
  const renderStatusBadge = () => {
    ...
    const getStatusColor = (code: ProposalStatusCode) => {
      switch (code) {
    ...
    return (
      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getStatusColor(status.code)}`}>
        {status.name}
      </span>
    );
  };
